...
app.use('/samplePage', async (req, res, next) => {
    ...
    try {
        ...
        fs.readFile(path.resolve('./build/index.html'), 'utf-8', (err, data) => {
            ...
            return res.send(replacedDatA.replace('<div id="root"></div>', `<div id="root">${ReactDomServer.renderToString(<App />)}</div>`))
...
app.get('/listUsers', (req, res) => {
    ...
    fs.readFile(path.resolve('server') + "/" + "users.json", 'utf8', (err, data) => {
        console.log(path.resolve('server') + "/" + "users.json", data, 'data', )
        res.end(data)
    })
